...
KB.tell(And(P,Q))
KB.tell(Implies(P, R)) #Kéo theo
KB.tell(Implies(And(Q,R),S))
print(KB.ask(S))'''

'''Trong một cuộc thi đấu cờ, ba bạn Trí, Dũng, Minh đạt cả ba giải cao nhất. Trí không đạt giải nhất, 
Dũng không đạt giải nhì, Minh không đạt giải nhất và không đạt giải ba. 
Hỏi ai đạt giải nhất, giải nhì và giải ba?'''


T1 = Atom("Tri dat giai nhat")
T2 = Atom("Tri dat giai nhi")
T3 = Atom("Tri dat giai ba")
D1 = Atom("Dung dat giai nhat")
D2 = Atom("Dung dat giai nhi")
D3 = Atom("Dung dat giai ba")
M1 = Atom("Minh dat giai nhat")
M2 = Atom("Minh dat giai nhi")
M3 = Atom("Minh dat giai ba")

# Build KB
KB  = createResolutionKB()
KB.tell(Not(T1))
KB.tell(Not(D2))
KB.tell(And(Not(M1),Not(M3)))

KB.tell(OrList([T1,T2,T3])) # Những giải mà Trí có thể đạt được
KB.tell(OrList([D1,D2,D3])) # Những giải mà Dũng có thể đạt được
KB.tell(OrList([M1,M2,M3])) # Những giải mà Minh có thể đạt được

KB.tell(AndList([T1, Not(T2), Not(T3), Not(D1), Not(M1)]))
KB.tell(AndList([T2, Not(T1), Not(T3), Not(D2), Not(M2)]))
KB.tell(AndList([T3, Not(T1), Not(T2), Not(D3), Not(M3)]))

KB.tell(AndList([D1, Not(D2), Not(D3), Not(T1), Not(M1)]))
KB.tell(AndList([D2, Not(D1), Not(D3), Not(T2), Not(M2)]))
KB.tell(AndList([D3, Not(D1), Not(D2), Not(T3), Not(M3)]))

KB.tell(AndList([M1, Not(M2), Not(M3), Not(D1), Not(T1)]))
KB.tell(AndList([M2, Not(M1), Not(M3), Not(D2), Not(T2)]))
KB.tell(AndList([M3, Not(M1), Not(M2), Not(D3), Not(T3)]))

# Mỗi giải chỉ được 1 người
print(KB.ask(T1))
print(KB.ask(T2))
print(KB.ask(T3))
