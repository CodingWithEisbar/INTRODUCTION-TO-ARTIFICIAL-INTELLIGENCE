...
def King(x): return Atom('King', x)

'''Build KB'''
KB = createModelCheckingKB()
print(KB.tell(Person(John)))
print(KB.tell(King(Richard)))
print(KB.tell(Forall('$x', Implies(King('$x'), Person('$x')))))

print(KB.ask(Person('$x')))
"""
...
def Criminal(x): return Atom('Criminal', x)
def American(x): return Atom('American', x)
def Weapon(y): return Atom('Weapon', y)
def Sells(x,y,z): return Atom('Sells', x,y,z)
def Hostile(x): return Atom('Hostile', x)
def Missile(x): return Atom('Missile', x)
def Owns(x,y): return Atom('Owns', x, y)

KB.tell(Forall('$x', Forall('$y', Forall('$z', Implies(AndList([American('$x'), Weapon('$y'), Sells('$x', '$y', '$z'), Hostile('$x')]))))))
KB.tell(Forall('$x', Forall('$y', Forall(''))))
